...
describe('ParticipantsController', () => {
  ...
  const mockParticipantsService = {
    ...
    listParticipants: jest.fn(
      (eventId: string): Promise<ParticipantEntity[]> => {
        return Promise.resolve([
          {
            id: 1,
            user: {
  ...
  it('should call updateParticipantDetails', async () => {
    ...
    const eventId = '1234';
    const pid = 1;
    const result = await participantController.updateParticipantDetails(
      eventId,
      pid,
      user,
    );
  ...
  it('should call updateStatus', async () => {
    ...
    const result = await participantController.updateParticipantStatus(
      pid,
      status,
    );
    expect(result).toEqual({
