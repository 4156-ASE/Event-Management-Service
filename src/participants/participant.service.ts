...
export class ParticipantsService {
  ...
  async inviteParticipant(eventId: string, user: { first_name: string, last_name: string, email: string }): Promise<void> {
  ...
  async updateParticipant(eventId: string, pid: number, user: { first_name: string, last_name: string, email: string }): Promise<UserEntity> {
    const event = await this.eventRepository.findOne({ where: { id: eventId } });
  ...
  async deleteParticipant(eventId: string, pid: number): Promise<void> {
        const result = await this.participantRepository.delete({ user: { id: pid }, event: { id: eventId } });
  ...
  async listParticipants(eventId: string): Promise<ParticipantEntity[]> {
        return await this.participantRepository.find({ where: { event: { id: eventId } }, relations: ["user"] });
    }


  async updateStatus(eventId: string, pid: number, status: string): Promise<void> {
    const participant = await this.participantRepository.findOne({ where: { user: { id: pid }, event: { id: eventId } } });
